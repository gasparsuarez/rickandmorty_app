...
void main() {
  late GetCharactersByPageUseCase useCase;
  late MockDataRepository mockDataRepository;

  setUp(() => {
        mockDataRepository = MockDataRepository(),
        useCase = GetCharactersByPageUseCaseImpl(mockDataRepository),
      });

  group('GetAllCharactersByPage ->', () {
    test('Get list of characters when function with page [1] is called', () async {
      int page = 1;
      final responseApi = {
        "info": {
          "count": 826,
      ...
      final expectedList =
          (responseApi['results'] as List).map((e) => Character.fromJson(e)).toList();

      //Arrange
      when(() => mockDataRepository.getAllCharactersByPage(page: 1))
          .thenAnswer((_) async => expectedList);
      //Act
      final charactersList = await useCase.getAllCharactersByPage(page: page);
      //Assert
      expect(charactersList, expectedList);
    });

    test('Test throw an exception when id less than or equal to 0', () {
      int page = 0;
      expect(() => useCase.getAllCharactersByPage(page: page), throwsException);
    });
  });
}
