...
void main() {
  group('GetCharactersByPage Bloc ->', () {
    ...
    setUp(() {
      ...
      dataBloc = DataBloc(mockGetAllCharacters);
    });
    ...
    test('Initial state is DataLoadingState', () {
      expect(dataBloc.state, isA<DataLoadingState>());
    });

    blocTest(
      'Emit [DataLoadingState, DataLoadedState] when successful',
      build: () {
        when(() => mockGetAllCharacters.getAllCharactersByPage(page: 1))
    ...
    blocTest(
      ...
      act: (bloc) => bloc.add(LoadPageEvent(page: 1)),
      expect: () => [isA<DataLoadingState>(), isA<DataErrorState>()],
    );
  });
}
